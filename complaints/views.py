...
#             assignments__engineer=request.user,
#             assignments__is_active=True,
#             status='assigned'  # Only assigned complaints can be resolved
#         ),
#         pk=pk
#     )
...
#                 # ===========================================
#                 # 🔥 FIXED: Process parts with user price input
#                 # ===========================================
#                 total_parts_cost = 0
                
#                 for part_issue in issued_parts:
#                     # Get quantity and price from POST for this specific part
#                     used_qty = int(request.POST.get(f'used_qty_{part_issue.id}', 0))
#                     user_price = float(request.POST.get(f'price_{part_issue.id}', 0))
                    
#                     if used_qty > 0:
#                         # Check stock
#                         part = Part.objects.select_for_update().get(id=part_issue.part.id)
#                         if part.stock_quantity < used_qty:
#                             messages.error(
#                                 request, 
#                                 f"Insufficient stock for {part.name}. Available: {part.stock_quantity}"
#                             )
#                             return redirect('resolve_complaint', pk=pk)
                        
#                         # Update part issue with user-provided price
#                         part_issue.used_quantity = used_qty
#                         part_issue.usage_status = 'used'
#                         part_issue.part_cost_at_time = user_price  # 💰 User entered price
#                         part_issue.save()
...
#                         # Calculate cost using user price
...
#                 # Grand total auto-calculates in save() method
#                 complaint.save()
